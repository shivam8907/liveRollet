...
using System.Threading.Tasks;
    public class PDFManager : MonoBehaviour
    {
        ...
        public static PDFManager CreatePDFBuilderWithPathAndTicketID(string path)
        {
       ...
       public PDFManager PdfToImage()
        ...
        //new printing data functionality
         public void PrintPdf() {
            try {
                // Define log file path (Desktop)
                string logFilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "sumatralogs.txt");
                // Log the start of the printing attempt
                LogToFile(logFilePath, "Attempting to print PDF: " + path);
                // Validate PDF file path
                if (string.IsNullOrEmpty(path) || !File.Exists(path)) {
                    string errorMessage = "Invalid file path: " + path;
                    LogToFile(logFilePath, errorMessage);
                    UnityEngine.Debug.LogError(errorMessage);
                    return;
                }
              // Path to SumatraPDF executable (relative to StreamingAssets)
string sumatraPath = Path.Combine(Application.streamingAssetsPath, "ThirdParty/SumatraPDF/SumatraPDF.exe");

// Log the constructed path
UnityEngine.Debug.Log("Checking for SumatraPDF executable at: " + sumatraPath);

// Check if the executable exists
if (!File.Exists(sumatraPath)) {
    string errorMessage = "SumatraPDF executable not found at: " + sumatraPath;
    LogToFile(logFilePath, errorMessage);
    UnityEngine.Debug.LogError(errorMessage);
    return;
}
                // Log the SumatraPDF executable path
                LogToFile(logFilePath, "Found SumatraPDF executable at: " + sumatraPath);
                // Print the PDF
                Process.Start(sumatraPath, $"-print-to-default \"{path}\"");
                string successMessage = "PDF sent to printer using SumatraPDF.";
                LogToFile(logFilePath, successMessage);
                UnityEngine.Debug.Log(successMessage);
            } catch (System.Exception ex) {
                // Log any exceptions to the log file
                string errorMessage = "Error printing PDF: " + ex.Message;
                LogToFile(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "sumatralogs.txt"), errorMessage);
                UnityEngine.Debug.LogError(errorMessage);
            }
        }
        // Helper method to write log to a text file
        private void LogToFile(string filePath, string message) {
            try {
                ...
                UnityEngine.Debug.LogError("Error writing to log file: " + ex.Message);
            }
        }
